...
class monitor_adb(object):
    ...
    def run_adb(self,adb_command,timeout):
        print (time.strftime("%Y-%m-%d %H:%M:%S"),"adb -s %s %s" % (self.deviceid,adb_command),timeout)
        self.p_cmd = subprocess.Popen("adb -s %s %s" %(self.deviceid,adb_command),shell=True,
    ...
    def kill_timeout_pid(self):
        self.command_pid = self.p_cmd.pid
        os.system("taskkill /t /f /pid %s" % self.command_pid)
        print("timeout process has killed")

def install_apk(apk_path):
    apk_version_error = "failure [install_failed_version_downgrade]"
    out , err = _monitor_adb.run_adb("install -r %s" % apk_path,60)
...
def install_uiautomator():
    ...
    time.sleep(5)
    install_apk(os.getcwd() + '/app-uiautomator.apk')
    time.sleep(5)
    install_apk(os.getcwd() + '/app-uiautomator-test.apk')
    time.sleep(5)
...
def cu_flash():
    ...
    while True:
        ser.write('\n\n')
        time.sleep(0.1)
        get_data = str(ser.read_all())
        if get_data.find(key_word) != -1:
            ser.write("cu" + '\n')
            print("input cu command finish")
            break
    time.sleep(10)
    ...
    for updatePage in range(30):
        ...
        try:
            if tv(resourceId=btn_id).exists:
                ...
                if action_btn_text in [u"Check for update", u"检查更新", u"Retry download","Check for update", "检查更新", "Retry download"]:
                    print("click update button")
                    action_btn.click()
                    time.sleep(60)
                    logger.info("Check the latest version ")
                elif action_btn_text in [u"Resume", u"继续", u"重新下载", u"Download", u"下载", u"Retry download", u"Resume",
                                     "继续", "重新下载", "Download", "下载", "Retry download"]:
                    print("click download button")
                    action_btn.click()
                    logger.info("Find the latest version,downloading")
                    time.sleep(120)
                elif action_btn_text in [u"Pause", u"暂停", "Pause", "暂停"]:
                    logger.info("The version is now being downloaded. ")
                    time.sleep(120)
                elif action_btn_text in [u"Restart now", u"立即重启", "Restart now", "立即重启"]:
                    action_btn.click()
                    ...
                    time.sleep(3)
                    logger.info("unkown button:%s" % action_btn_text)
                    continue
            else:
...
def write_ser_data():
    ...
    while True:
        read_ser_log = str(read_ser_data().next())
...
if __name__ == '__main__':
    ...
    ser_log = result_dir + "/" + device_id + "_" + ser_num + "_log.log"
    ...
    ser = serial.Serial()
    t = threading.Thread(target = write_ser_data).start()
    start()
